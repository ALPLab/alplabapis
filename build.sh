...
CURRENT_BRANCH=${CIRCLE_BRANCH-"AUTOGEN"}
...
function buildProtoForTypes {
  ...
  if [ -f .protolangs ]; then
    while read lang; do
      ...
      echo "Cloning repo: git@github.com:ALPLab/$reponame.git"

      # Clone the repository down and set the branch to the automated one
      git clone git@github.com:ALPLab/$reponame.git $REPOPATH/$reponame
      ...
      # that we care about
      cp -R `pwd`/gen/pb?$lang/* $REPOPATH/$reponame/
